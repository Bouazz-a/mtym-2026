...
// algorithm) per Latin square instead of enumerating every problem
// permutation, and is *best-effort*: it never throws when a perfect
// (score 0) solution is impossible. Instead it keeps the globally
// minimum-penalty plan it found and publishes a ConstraintReport that
...
// re-implemented here so this module is fully self-contained.
//
...
function generateRound2BestEffort(
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    ...
    for (let poolIdx = 0; poolIdx < composition.length; poolIdx++) {
      ...
      total += solved.score;
      plans.push({
        pool: { id: uuidv4(), label: buildPoolLabel(2, poolIdx), round: 2 },
